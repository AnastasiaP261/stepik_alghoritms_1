...
время _n log(n)_.  
В этом Алгоритме мы используем 3 массива d, pos, prev (пояснения к ним в тексте программы). Линейно проходим по массиву,
...
def search_ls(a):
    ...
    d = [10**9 + 1] * (n + 2)
    d[0] *= -1
    pos = [''] * (n + 1)                    # индексы эл-тов, на которые заканчивается П длины i
